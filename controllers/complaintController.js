const { stakingContract, complaintContract } = require("../services/blockchain");
const { uploadJSON } = require("../services/filecoin");
...
module.exports = {
  fileComplaint: async (req, res) => {
    try {
      ...
      if (!staked) return res.status(400).json({ message: "User must stake 0.001 ETH first" });
